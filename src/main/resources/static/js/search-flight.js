...
const flightError = document.querySelector("#flight-error");
...
const getResultsBtn = document.querySelector("#get-results-btn");
...
// Clears departure date error message
clearErrors(flight, flightError);
...
getResultsBtn.addEventListener("click", (event) => {
  ...
  if (isValid) {
    document.querySelector("#search-flight-form").submit();
  }
});
...
if(reviewBtn) {
	reviewBtn.addEventListener("click", (event) => {
	  ...
	  //validates that the flight is selected
	  if (!validate(flight, flightError, "Flight")) isValid = false;
